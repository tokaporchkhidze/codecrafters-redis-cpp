...
ArgSpec LpushCommand::arg_spec() const { return {2, std::nullopt}; }

ExecutionOutcome LpushCommand::execute(std::span<std::string const> const args,
                                       CommandContext const &,
                                       CommandDeps &deps)
{
  if (auto const new_size{deps.store->lpush(args[0], args.subspan(1))};
      new_size.has_value()) {
